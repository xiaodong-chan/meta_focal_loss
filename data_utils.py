...
# 字典：每类数据的index
    ...
    train_data_meta.targets = np.delete(train_dataset.targets, idx_to_train, axis=0).astype('int64')
    train_data.data = np.delete(train_dataset.data, idx_to_meta, axis=0)
    train_data.targets = np.delete(train_dataset.targets, idx_to_meta, axis=0).astype('int64')
