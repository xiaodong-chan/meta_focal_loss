...
def main():
    ...
    #optimizer_c = torch.optim.SGD(anet.params(), 1e-5,momentum=args.momentum, nesterov=args.nesterov,weight_decay=args.weight_decay)
    ...
    for epoch in range(args.epochs):
        ...
        train(imbalanced_train_loader, validation_loader, model, anet, optimizer_a, epoch)
...
def train(train_loader, validation_loader, model, anet, optimizer_a, epoch):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        cross_cost = F.cross_entropy(y_f_hat, target_var,reduction='none')
        cost_v = torch.reshape(cross_cost, (len(cross_cost), 1))
        p = torch.exp(-cost_v)
        v_lambda = anet(p)
        #l_f_meta = focal_loss(y_f_hat, target_var, v_lambda)
        ...
        input_validation_var = input_validation.cuda()
        ...
        with torch.no_grad():
            for p in anet.parameters():
                p -= p.grad * (1e-2 * 0.1 ** (epoch // 20))
            anet.zero_grad()

        y_f = model(input_var)
        cost_w = F.cross_entropy(y_f, target_var, reduction='none')
        cost_n = torch.reshape(cost_w, (len(cost_w), 1))
        p_n = torch.exp(-cost_n)
        ...
        if norm_v != 0:
            ...
            w_v = w_new
        l_f = torch.sum(cost_n * w_v)
        #l_f = torch.sum(cost_new)
        ...
        l_f.backward()
        optimizer_a.step()
        expos = anet.expo.data.to(torch.device('cpu')).numpy()[0]
        ...
        if i % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                ...
                epoch, i, len(train_loader), expos,
